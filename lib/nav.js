...
var menu = document.getElementById('menu-toggle'),
  ...
  function renderSmallCart(cart){
    ...
    let str = '';
    $('.dropdown-menu.cart-badge .sally').html(str);
    let cartl = Object.keys(cart).length;
    // let taco = $('.dropdown-menu.cart-badge').find("a");
    let taco = $(".dropdown-menu.cart-badge:has(img)").length

    console.log("has or img:" + taco);
    console.log("length:length:length:length:" + cartl);
    
    if (cartl == 0 && taco == 0) {
      // let content = $('.dropdown-menu.cart-badge').html()
      $('.dropdown-menu.cart-badge .sally').html("<div class='product-info ml-3' style='font-size: 1rem;'>購物車沒有商品</div>");
    ...
    for (let i in cart) {
      ...
      // let content = $('.dropdown-menu.cart-badge .sally').html();
    $('.dropdown-menu.cart-badge .sally').html(str);
...
//   const customTpl = function(custom_cart){
//     return `
