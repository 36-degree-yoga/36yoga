...
var menu = document.getElementById('menu-toggle'),
  ...
  function renderSmallCart(cart){

    let str = '';
    let cartl = Object.keys(cart).length;
    console.log("length:length:length:length:" + cartl);
    
    if(cartl==0){
      $('.dropdown-menu.cart-badge').html("<div class='product-info ml-3' style='font-size: 1rem;'>購物車沒有商品</div>");
    ...
    for (let i in cart) {
      ...
      str += productTpl(cart[i]);
    
    }
    
    $('.dropdown-menu.cart-badge').html(str);
      
    }
    
  }

  $.get('handle-cart-product.php', function(data) {
    console.log(data);
    // countCart(data.cart);
    renderSmallCart(data.cart);
    CrenderSmallCart(custom_cart);
  }, 'json');
  ...
  const customTpl = function(c){
    return `
    <a class="d-flex " href="#">
    <div class="img-wrap border" style="background-color: ${c.color};">
        <img class="product-pic" style="" src="img/customize/design/${c.img}.png" alt="">
    </div>
    <div class="product-info ml-3">
        <p>${c.product_name} <br>NT$ ${c.price}</br> </p>
    </div>
</a>
    `;
};

  function CrenderSmallCart(custom_cart){

    let cstr = '';
    for (let i in custom_cart) {
      cstr += customTpl(custom_cart[i]);
    }
    let content = $('.dropdown-menu.cart-badge').html();
    $('.dropdown-menu.cart-badge').html(content+cstr);
  }

  $.get('handle-custom.php', function(data) {
    console.log(data);
    // countCart(data.cart);
    CrenderSmallCart(data.custom_cart);
